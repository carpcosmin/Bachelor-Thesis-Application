...
public class FragmentBuy extends Fragment {
    ...
    private List<Offer> offersToSell = new ArrayList<>();
    User currentUser;
        ...
        if (token != null) {
            ...
            userAPI.getUserByToken(token).enqueue(new Callback<User>() {
                ...
                public void onResponse(Call<User> call, Response<User> response) {
                    ...
                    offersRV.setLayoutManager(new LinearLayoutManager(FragmentBuy.super.getContext()));
                    OfferAdapter offerAdapter = new OfferAdapter(offerList, currentUser, getContext());
                    ...
                    offerAPI.getAllOffers().enqueue(new Callback<List<Offer>>() {
                        ...
                        public void onResponse(Call<List<Offer>> call, Response<List<Offer>> response) {
                            ...
                            // display only the offers which are marked as "Sell" (the ones which are not to "Rent")
                            offersToSell.clear();
                            for (Offer offer : offerList) {
                                Boolean toRent = offer.getToRent();
                                if (toRent != null && !toRent) {
                                    offersToSell.add(offer);
                                }
                            }

                            OfferAdapter offerAdapter = new OfferAdapter(offersToSell, currentUser, getContext());
                            offersRV.setAdapter(offerAdapter);
                            ...
                            offerAdapter.setOnItemClickedListener(new OfferAdapter.OnItemClickListener() {
                                ...
                                public void onItemClick(int position) {
                                    Intent viewOfferIntent = new Intent(FragmentBuy.super.getContext(), ViewOfferActivity.class);
                                    // sending the id of the offer to the View Activity
                                    viewOfferIntent.putExtra("offerId", offerAdapter.offersList.get(position).getId());
